...
function LoadingState() {
  return (
    <div className="flex flex-col gap-3 p-6" role="status" aria-label="Loading issues">
      ...
      <div className="border-b border-border px-4 py-3 sm:px-6">
        <div className="flex items-center gap-3">
          <ToggleGroup
            value={stateFilter}
            onValueChange={(value) => {
              if (value) setStateFilter(value as "open" | "closed" | "all");
            }}
            type="single"
            size="sm"
              ...
              >
                <CardHeader>
                  <div className="flex items-start gap-2.5">
                    {issue.state === "open" ? (
                      <CircleIcon className="mt-0.5 size-4 shrink-0 text-emerald-500" aria-label="Open issue" />
                    ) : (
                      <CheckCircle2Icon className="mt-0.5 size-4 shrink-0 text-muted-foreground" aria-label="Closed issue" />
                    )}
                      ...
                      <div className="mt-1.5 flex flex-wrap items-center gap-1.5" role="list" aria-label="Labels">
                        {issue.labels.map((label) => (
                          <Badge
                            ...
                            variant="outline"
                            size="sm"
                            className={LABEL_COLORS[label.color] ?? ""}
                            role="listitem"
                          >
                            {label.name}
                          </Badge>
                ...
                <CardPanel>
                  <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                    {issue.assignee ? (
                      <span className="flex items-center gap-1" aria-label={`Assigned to ${issue.assignee.login}`}>
                        <UserIcon className="size-3.5" aria-hidden="true" />
                        {issue.assignee.login}
                      </span>
                    ) : (
                      <span className="flex items-center gap-1 text-muted-foreground/60" aria-label="Unassigned">
                        <UserIcon className="size-3.5" aria-hidden="true" />
                        Unassigned
                      </span>
                    )}
